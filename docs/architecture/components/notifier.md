...
- Building the current session link,
  `http://127.0.0.1:3000/sessions/<session_id>`, so a notification click can
  open that session in the local Inspector.
...
- The current session action opens the local Inspector URL. The notifier does
